Source: https://acmecse.net/setup/Operation-uppertester/
...
Test (SUT), ie. a CSE, to perform certain oneM2M operations and other actions. See oneM2M's [TS-0019 "Abstract Test Suite and Implementation eXtra Information for Test"](https://specifications.onem2m.org/ts-0019) specification for further details.

To support this feature an additional endpoint `__UT__` is available under the HTTP server's root path. It can be enabled by setting the configuration [`[http].enenableUpperTesterEndpoint`](../Configuration-http/#general-settings) in the configuration file to `True`.
...
The *Upper Tester* endpoint currently only supports a limited set of the functionality specified in [TS-0019](https://specifications.onem2m.org/ts-0019), but offers additional functionality, such as sending commands with arguments and receiving return values.
...
as [scripts](../../development/ACMEScript/) that have the meta tag [@uppertester](../../development/ACMEScript-metatags/#uppertester) set.
...
| disableShortRequestExpiration | For running [test cases](../../development/UnitTests/): Disables short request expiration. |
| disableShortResourceExpiration | For running [test cases](../../development/UnitTests/): Disables short resource expiration. |
| enableShortRequestExpiration | For running [test cases](../../development/UnitTests/): Enables short request expiration. |
| enableShortResourceExpiration | For running [test cases](../../development/UnitTests/): Enables short resource expiration. |
