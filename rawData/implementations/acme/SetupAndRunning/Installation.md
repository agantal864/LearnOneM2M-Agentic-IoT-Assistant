Source: https://acmecse.net/setup/Installation/
...
Using pip (Package Installation)Manual Installation
...
1. Install the ACME CSE by cloning the repository, or by downloading the [latest](https://github.com/ankraft/ACME-oneM2M-CSE/releases/latest) release package, unpacking it, and copying the whole distribution to a new directory.
   ...
   cd ACME-oneM2M-CSE
   ```
2. It is recommended to install the required packages by running the following command:
...
1. Run the CSE for the first time.  
   ...
   For Package Installation For Manual Installation
   ...
   Running the ACME CSE as a module

   ```
   python3 -m acme
   ```

   Please refer to the [Running](../Running/) documentation for more detailed instructions how to start and run the ACME CSE.
...
2. After terminating the CSE again you can edit that configuration file and add more settings if necessary.
   ...
   See the [Configuration](../Configuration-introduction/) documentation for further details, and the default configuration file [acme.ini.default](https://github.com/ankraft/ACME-oneM2M-CSE/blob/master/acme/init/acme.ini.default).
...
Running the onboarding tool as a module
...
If you want to use a [Zookeeper configuration service](../Configuration-introduction/#using-apache-zookeeper-for-configuration) instead of a local configuration file to manage the configuration of the CSE, you can create a configuration that is stored in Zookeeper. This can be done by passing the *--zookeeper-host* (and optionally *--zookeeper-port*) command line arguments to the onboarding tool:

For Package InstallationFor Manual Installation

Running the onboarding tool with Zookeeper

```
acmecse-onboarding --zookeeper-host <host>
```

Running the onboarding tool with Zookeeper as a module
...
Storing the configuration in Zookeeper is also possible when running the CSE for the first time. When you run the CSE with the [--config-zk-host](../Running/#command-line-arguments) command line argument, the CSE will start the onboarding process and store the configuration in Zookeeper.
...
Installing the ACME CSE Development Version
