Source: https://acmecse.net/setup/Configuration-logging/
...
| filter | List of component names to exclude from logging. | werkzeug,markdown\_it,asyncio |
| level | Loglevel. Allowed values: `debug`, `info`, `warning`, `error`, `off`. See also command line argument [–log-level](../Running/#command-line-arguments). | debug |
...
| path | Pathname for log files. | [${basic.config:baseDirectory}](../Configuration-basic/#basic-configuration)/logs |
