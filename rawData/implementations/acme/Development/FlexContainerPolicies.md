Source: https://acmecse.net/development/FlexContainerPolicies/
...
For all <flexContainer> specializations, e.g. for oneM2M's TS-0023 ModuleClasses, the [attribute policies](../AttributePolicies/) and the allowed <flexContainer> hierarchy must be provided.

The files for <flexContainer> specializations are also automatically imported from the [init](https://github.com/ankraft/ACME-oneM2M-CSE/blob/master/acme/init) and [secondary init](../../setup/Running/#secondary-init-directory) directory. More than one such file can be provided, for example one per domain. The files must have the extension `.fcp`.
...
The *attributePolicies* are the same as described in the [Attribute Policies](../AttributePolicies/) article.

See also oneM2M's [TS-0023 specification rules](https://specifications.onem2m.org/ts/ts-0023/latest/5.2/#522-description-rules-for-module-classes-and-deviceclasses) for more information when defining new ModuleClasses and DeviceClasses.
...
It is possible to define own specializations that need to be placed in schema files, similar to the provided ones. These files must have the `.fcp` extension and must be placed in one of the [init](https://github.com/ankraft/ACME-oneM2M-CSE/blob/master/acme/init) or [secondary init](../../setup/Running/#secondary-init-directory) directories. They are imported automatically when the CSE starts.
