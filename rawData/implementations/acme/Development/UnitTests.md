Source: https://acmecse.net/development/UnitTests/

# Unit Tests

Almost all aspects of the ACME CSE implementation are covered by unit tests based on the Python [unittest](https://docs.python.org/3/library/unittest.html) framework. The files for the individual test suites and the runner application reside in the [project's *tests*](https://github.com/ankraft/ACME-oneM2M-CSE/tree/master/tests) directory.

## Configuration

The actual configuration of the test suite is done in the file [config.py](https://github.com/ankraft/ACME-oneM2M-CSE/blob/master/tests/config.py). You may change these for your individual set-up. At the top of the configuartion is a configuration setting for the request protocol that should be used. Currently, *http*, *https*, *ws*, *wss*, and *mqtt* are supported.
...
The CSE under test can be started with the **remote configuration interface** enabled. During test runs the test suite will temporarily change some of the CSE's delays (e.g. the check for resource expirations) in order to speed up the test. You can either do this by changing the configuration [http.enableUpperTesterEndpoint](../../setup/Configuration-http/#general-settings) in the CSE's [configuration file](https://github.com/ankraft/ACME-oneM2M-CSE/blob/master/tests/config.py).
...
Each test suite imports the file [init.py](https://github.com/ankraft/ACME-oneM2M-CSE/blob/master/tests/init.pys) that contains various helper functions. Also, some low-level configurations, such as time-outs etc, that are used by the test suites can be adjusted here.
...
The individual test suites are located in the [tests](https://github.com/ankraft/ACME-oneM2M-CSE/tree/master/tests) directory. Their names start with *test...* and are grouped by the aspect of the CSE they are testing.
...
Run Single Test Case Without Tear-Down
...
The following example runs all test cases in the *testSUB* test suite except the *test\_createCNTforEXC* test case:
