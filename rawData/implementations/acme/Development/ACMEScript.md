Source: https://acmecse.net/development/ACMEScript/
...
* Implement tool scripts for the [Text UI](../../setup/TextUI/).

## ACMEScript Basics

The scripts are stored as normal text files. A script contains so-called [s-expressions](https://en.wikipedia.org/wiki/S-expression) that are evaluated one by one and recursively.
...
Sometimes it is not possible to quote an s-expression or symbol because it is the result of the evaluation of another s-expression. In this case the [quote](../ACMEScript-functions/#quote) function can be used to return a quoted version of an s-expression.
...
Meta tags are described in [a separate documentation](../ACMEScript-metatags/).
...
See: [get-storage](../ACMEScript-functions/#get-storage), [has-storage](../ACMEScript-functions/#has-storage), [put-storage](../ACMEScript-functions/#put-storage)
...
Evaluation can also be disabled and enabled by using the [evaluate-inline](../ACMEScript-functions/#evaluate-inline) function.
