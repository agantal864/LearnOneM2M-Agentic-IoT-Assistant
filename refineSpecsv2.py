...
class qaType(BaseModel):
    ...
    fact_type: str = Field(..., description="definition | list | mapping | constraint | protocol | reference")
...
def textSplitAndRefine(filePath):
    ...
    for doc in md_header_splits:
        # Split ONLY within the current header block
    ...
    # Refining
    ExtractedQA = []
    for section in finalSplits:
        # Add metadata to every section.page_content (Default: General)
        metadata = section.metadata if section.metadata else {"header": "General", "level": 0}
        headerLevel = metadata.get("level", 0)
        sectionTitle = metadata.get("header", "General")
        ...
        while True:
            try:
                ...
                if "rate limit" in str(e).lower() and retries < maxRetries:
                    waitTime = 2 * (2 ** retries)  # start with 2 seconds, could increase each retry
                    print(f"Rate limit hit, retry {retries+1}/{maxRetries}, sleeping {waitTime}s...")
                    time.sleep(waitTime)
                    retries += 1
                else:
                    raise
    ...
    print(f"\nProcessed: {jsonFilename} | Total Q&A: {len(ExtractedQA)}")
    return ExtractedQA
...
#             For every Q&A pair, assign ONE of the following fact_type values:
#             - definition:
#             Use when defining a term, acronym, or concept.
#             Example: "What is AE?" → definition
#             - list:
#             Use when enumerating supported values or options.
#             Example: "Which HTTP methods are supported?" → list
#             - mapping:
#             Use when mapping one value to another.
#             Example: "What HTTP Status maps to oneM2M code 2001?" → mapping
#             - constraint:
#             Use for MUST / SHALL / MAY rules, limits, conditions.
#             Example: "Is caching supported?" → constraint
#             - protocol:
#             Use for message formats, headers, flows, transport behavior.
#             Example: "What is the format of the CoAP message header?" → protocol
#             - reference:
#             Use for RFCs, TS documents, and citations.
#             Example: "What is the title of IETF RFC 7252?" → reference
#         RULE: If unsure between two types, choose the more specific one.
#         CRITICAL CONSTRAINT:
...
#     """
#     Filters out duplicates, N/A responses, and pointers to clauses/tables, Circular Q&A (question ~ answer), Section-page noise (e.g., "7.2Transport Layer19")
#     """
#     uniquePairs = {}
...
#         r"as\s+described\s+in", r"figure\s+\d", r"appendix\s+\w"
#     ]
#     # Pattern for "7.2Transport Layer Security19"
#     sectionPagePattern = r"^\d+[\d\.]*[A-Z][a-zA-Z\s\-\_]+\d+$"
#     validFactType = {"definition", "list", "mapping", "constraint", "protocol", "reference"}
#     for entry in data:
#         qText, aText = entry['q'].strip(), entry['a'].strip()
        
#         # 1. Length & N/A Filter
#         if qText.upper() == "N/A" or aText.upper() == "N/A":
#             removedCount["naFilter"] += 1
#             continue
        
#         # 2. Fact type validation
#         if 'fact_type' not in entry:
#             continue
#         entry['fact_type'] = entry['fact_type'].strip().lower()
#         if entry['fact_type'] not in validFactType:
#             continue

#         # 3. Forbidden Phrases Filter
#         if any(re.search(pat, aText, re.IGNORECASE) for pat in forbiddenPatterns):
#             removedCount["forbiddenPattern"] += 1
#             continue

#         # 4. Section-Page Noise Filter (The '7.2Security19' issue)
#         if re.match(sectionPagePattern, aText.replace(" ", "")):
#             removedCount["sectionPageNoise"] += 1
#             continue
        
#         # 5. Fuzzy Filter: Remove circular Q&A (Question and Answer too similar)
#         similarity = difflib.SequenceMatcher(None, qText.lower(), aText.lower()).ratio()
#         if similarity > 0.85:
#             removedCount["fuzzyDuplicate"] += 1
#             continue

#         # 6. Deduplication and Category Normalization
#         # Using (Question + Answer) as the key to catch duplicates
#         key = (qText.lower(), aText.lower())
#         if key in uniquePairs:
#             removedCount["dedup"] += 1
#             continue
#         else:
...
#             entry['category'] = f"{entry['fact_type']} | {entry['category']}"
